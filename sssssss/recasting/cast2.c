...
/*   Updated: 2025/03/24 14:47:57 by asebrani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	choose_closest_hit(t_rays *ray, t_hit_points *hit, t_player *player)
{
	...
	vd = FLT_MAX;
	if (hit->found_horz)
		hd = calculate_distance(player->x, player->y, hit->horz_x, hit->horz_y);
	if (hit->found_vert)
		vd = calculate_distance(player->x, player->y, hit->vert_x, hit->vert_y);
	if (hd < vd)
	{
		ray->distance = hd;
