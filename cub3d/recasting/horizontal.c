...
/*   Updated: 2025/03/24 15:14:56 by asebrani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_horizontal_hit(t_mlx *mlx, t_rays *ray, float next_x, float next_y)
{
	int	map_x;
	int	map_y;
		...
		float ray_angle, t_hit_points *hit)
{
	...
	{
		if (is_horizontal_hit(mlx, ray, next_x, next_y))
		{
			hit->horz_x = next_x;
			hit->horz_y = next_y;
			hit->found_horz = 1;
