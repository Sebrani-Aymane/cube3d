...
/*   Updated: 2025/03/24 15:19:55 by asebrani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	is_vertical_hit(t_mlx *mlx, t_rays *ray, float next_x, float next_y)
{
	int	map_x;
	int	map_y;
		...
		float ray_angle, t_hit_points *hit)
{
	...
	{
		if (is_vertical_hit(mlx, ray, next_x, next_y))
		{
			hit->vert_x = next_x;
			hit->vert_y = next_y;
			hit->found_vert = 1;
